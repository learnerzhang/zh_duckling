...
module Duckling.Weight.Types where
...
instance Resolve WeightData where
  type ResolvedValue WeightData = WeightValue
  resolve _ _ WeightData {unit = Just unit, value = Just val} =
    Just (simple unit val, False)
  resolve _ _ WeightData {unit = Just unit, value = Nothing
...
instance ToJSON WeightValue where
...
simple :: Unit -> Double -> WeightValue
simple u v = SimpleValue $ single u v

between :: Unit -> (Double, Double) -> WeightValue
between u (from, to) = IntervalValue (single u from, single u to)

above :: Unit -> Double -> WeightValue
above = openInterval Above

under :: Unit -> Double -> WeightValue
under = openInterval Under

openInterval :: IntervalDirection -> Unit -> Double -> WeightValue
